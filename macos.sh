...
traceinfo "Customizing MacOS settings"
...
function update() {
  if [[ $(defaults read /Library/Preferences/com.apple.SoftwareUpdate AutomaticCheckEnabled) != 1 ]]; then
    tracecommand "sudo defaults write /Library/Preferences/com.apple.SoftwareUpdate AutomaticCheckEnabled -int 1"
  fi
  if [[ $(defaults read /Library/Preferences/com.apple.commerce AutoUpdate) != 1 ]]; then
